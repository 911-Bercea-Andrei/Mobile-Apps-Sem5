...
class UpdatePlantActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.updateLastWateredEditText.setText(plant.lastWatered.toString())
        ...
        binding.updateSaveButton.setOnClickListener {
            ...
            val newWateringInterval = binding.updateWateringIntervalEditText.text.toString().toIntOrNull() ?: 0

            val updatedPlant = Plant(plantId, newName, newLastWatered, newWateringInterval)
            db.updatePlant(updatedPlant)
            finish()
